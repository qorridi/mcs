...
Future<void> uploadFile() async {
  ...
  if (result != null) {
    ...
    var req = http.MultipartRequest(
        "POST", Uri.parse("http://10.3.4.231:8081/file"));
...
Future<List<dynamic>> getFile(nm) async {
  var response = await http.get(Uri.parse('http://10.3.4.231:8081/file/$nm'));
  return jsonDecode(response.body)['data'];
}

Future<bool> delFile(id) async {
  final response = await http
      .delete(Uri.parse('http://10.3.4.231:8081/file/$id'), headers: {
